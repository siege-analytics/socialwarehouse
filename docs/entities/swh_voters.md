...
- **Chunk-append is column-shape-checked.** Post-S4/SW#134 fix: `load_voter_file` records the columns of the first non-empty chunk and refuses any subsequent chunk whose column set differs (raises `ValueError` naming S4/SW#134 + which columns were added/removed). The staging table is then cleaned up by the existing exception handler. Pre-fix this was masked by SU#517's column-loss bug (only `geom` was written); post-SU#517 columns are written by name and chunk drift would either raise (extra columns) or NaN-fill silently (missing columns). The explicit check happens BEFORE the upload so the misalignment never reaches the database.
...
- **Cleanup-exception warning includes manual remediation SQL.** Post-S8/SW#138 fix: if staging-table cleanup fails after a load failure, the warning names the qualified table AND emits the literal `DROP TABLE IF EXISTS schema.staging_table;` an operator can paste into psql to finish the cleanup. Pre-fix the warning named the bare staging table only.
...
- 2026-05-19: S6 / SW#136 fix — added `_validate_identifier` helper, applied to `schema`, `table_name`, and `staging_table` before any I/O. Identifiers are rejected if they don't match `^[A-Za-z_][A-Za-z0-9_]*$` (bare ASCII), then double-quoted when interpolated into LOCK / DROP / ALTER TABLE DDL. Closes the injection vector on the `--schema` / `--table` CLI options.
- 2026-05-19: S8 / SW#138 fix — cleanup-exception warning now includes the literal `DROP TABLE IF EXISTS schema.staging_table;` SQL the operator can paste into psql. Pre-fix the warning named only the bare table name. S4 remains open (paused pending SU#517 merge).
- 2026-05-19: S4 / SW#134 fix — explicit chunk-shape check in `load_voter_file`. First non-empty chunk's columns are recorded; subsequent chunks with a different column set raise `ValueError` (added/removed columns named in the message). Closes the chunk-drift misalignment vector that SU#517 (PostGISConnector now writes all columns) exposed.
