...
- 2026-05-19: ST5 / SW#143 + ST6 / SW#144 — added contract comment to `ALLOWED_HOSTS = []` explaining environments override; removed `P1B-B #68` history reference from the `locations` INSTALLED_APPS comment (writing-code:2). Settings is no longer the only place that needs sys.path documentation; the new comment is concise.
